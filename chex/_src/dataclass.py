...
def mappable_dataclass(cls):
  ...
  # Check Mappable mixin methods
  # If a class implements these they will be used over the default
  # implementations from Mappable
  mappable_mixins = ("__contains__", "__eq__", "__ne__")
  implemented_mixins = [m for m in mappable_mixins if m in cls.__dict__]
  if implemented_mixins:
    logging.warning(
        "Mappable dataclass %s implements %s. These will be used "
        "instead of the default abc.Mappable implementations", str(cls),
        implemented_mixins)
